...
NeuroControllerDriver::NeuroControllerDriver(int nIn, int nOut, vector<pair<double,double> > &oRanges)
{
    ...
    numNodesLayer = {nInputs+bias, nOutputs}; // No hidden layers. Adding pseudo input x0=1 for Bias.
   ...
   // Posicion palma = -0.294607, 0.0657, 0.172029
   // Posicion pelota = 0.47, 0.85, 0.73
   // DistXYZ = -0.764607, -0.7843, -0.557971
   // Distancia = 1.22926074991842149845
}
...
void NeuroControllerDriver::nextArmMove(vector<double> const &inputs, vector<double> &reaction)
{
   //cerr << "\n\nEntrando a nextArmMove...";
   /*** 0. Get the NN's input values from the current robot's state. ***/
   if (bias == 1) {
   layerOutputs[INPUT_LAYER][0] = 1.0;  // x0 = 1; For x0 * w0 = w0  (The bias)
   for (size_t i = 1; i < layerOutputs[INPUT_LAYER].size(); i++)
      layerOutputs[INPUT_LAYER][i] = inputs[i-1];
   }
   else
